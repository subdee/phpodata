	...
	public static function getUri($object) {
		...
		foreach ($object->getEntityKeys() as $entityKey) {
			...
			if (Utility::ContainAttribute($prop->getDocComment(), 'Edm.Int64') == TRUE) {
				$keyQueries[$entityKey] = str_replace(' ', "%20", $keyVal) . 'L';
			} else if (Utility::ContainAttribute($prop->getDocComment(), 'Edm.String') == TRUE) {
				$keyQueries[$entityKey] = "'" . str_replace(' ', "%20", $keyVal) . "'";
			} else {
				$keyQueries[$entityKey] = str_replace(' ', "%20", $keyVal);
			}
		}

		if (count($keyQueries) < 0) {
			return null;
		} else if (count($keyQueries) == 1) {
			...
			if ($i !== FALSE) {
				...
				if (isset($AttributeCollection[$key])) {
					if (! is_array($AttributeCollection[$key])) {
						// Converting to array if multiple attributes with same
						// name are found.
						// I don't like this. But a lot of code may rely on it.
