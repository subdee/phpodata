...
class SaveResult {
	...
	protected function CreateChangeSetBodyForResource($resourceBox, $newline) {
		...
		// Write properties to changeset
		$this->CreateBodyForProperties($type, $object, $changesetBodyForResource);
	...
	protected function CreateBodyForProperties($type, $object, &$body) {
		...
		foreach($nonEpmProperties as $nonEpmProperty) {
			...
			$refProperty = new ReflectionProperty($object, $propertyName);
			$propertyValue = $refProperty->getValue($object);
			$propertyAttributes = Utility::getAttributes($refProperty);
			$edmType = isset($propertyAttributes['EdmType']) ? $propertyAttributes['EdmType'] : "";
			$notNullable = isset($propertyAttributes['NotNullable']);

			//Now check for complex type. If type not start with 'Edm.'
			//it can be a complex type.
			if(strpos($edmType, 'Edm.') !== 0) {
				Utility::WriteLine('<d:' . $propertyName . ' m:type="' .
						$edmType . '">');

				$this->CreateBodyForProperties(ClientType::Create($propertyName), $propertyValue, $body);

				Utility::WriteLine('</d:' . $propertyName . '>');
				continue;
			}
